...
export function options(argv: string[], env = process.env) {
  const { values, positionals } = parseArgs({ args: argv, strict: true, allowPositionals: true, options: {
    ...
    "command-timeout": { type: "string" }, "max-output": { type: "string" },
  } });
  ...
  const number = (name: "context" | "max-tokens" | "max-turns" | "command-timeout" | "max-output",
  ...
  const config: Config = {
    ...
    maxTurns: number("max-turns", 32, 10000),
