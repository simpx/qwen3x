#!/usr/bin/env python3
"""把固定的 Qwen3.5-0.8B text backbone 转为 Stage 3 使用的顺序二进制文件。

这不是通用模型转换器。它只做一件事：以同目录 qwen35_cuda.cu 前向执行所需的
...
字节布局，以及 qwen35_cpu.cpp 中 Model 构造函数下一次 Reader.take() 应读到的内容。
"""
