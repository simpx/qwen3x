// qwen35_cuda.cu -- Qwen3.5-0.8B 的 plain CUDA + cuBLAS reference。
//
// 这是 Stage 3 的独立 snapshot。qwen35_cpu.cpp 是同目录的 CPU model loader/数学
// reference；本文件只把明确的 W*x 替换成 cuBLAS，并把 state 常驻 GPU。它仍只服务
// 一个固定模型、batch=1 和单 GPU：没有 Tensor、Backend class、registry 或 scheduler。
...
// Linear weights are uploaded once as FP32.  The packed checkpoint remains BF16 on disk and
// all non-linear parameters stay BF16 on GPU; this one explicit expansion lets the first CUDA
// stage use the same `BF16 weight value × FP32 activation` arithmetic as the CPU reference.
__global__ void k_bf16_to_f32(const B* input, float* output, size_t n) {
    const size_t i = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (i < n) output[i] = dbf(input[i]);
}
...
// The CPU `Linear` owns a pointer to BF16 mmap data.  CUDA's correctness snapshot deliberately
// expands just matrix weights to FP32, so its device-side layer structs say that difference out
// in the type rather than hiding it behind a cast or a generic Tensor class.
struct DeviceLinear { const float* w = nullptr; int rows = 0; int cols = 0; };
struct DeviceDelta {
    DeviceLinear qkv, z, a, b, out;
...
struct DeviceModel {
    ...
    std::array<DeviceLayer, N> layer {};

    explicit DeviceModel(const char* path) : host(path) {
        embedding = upload_bf16(host.embedding, static_cast<size_t>(V) * H);
        // Same checkpoint table, different runtime representation: embedding lookup reads BF16
        // directly, whereas the tied lm_head needs the FP32 linear-matrix representation.
        lm_head = upload_linear({host.embedding, V, H});
        final_norm = upload_bf16(host.final_norm, H);
        for (int index = 0; index < N; ++index) {
            const Layer& source = host.layer[index];
            DeviceLayer& target = layer[index];
    ...
    DeviceLinear upload_linear(const Linear& source) {
        const size_t count = static_cast<size_t>(source.rows) * source.cols;
        // This temporary is intentionally freed before loading the next matrix: runtime keeps
        // only the FP32 device matrix, not two copies of all linear weights.
        B* temporary = nullptr;
        cuda_ok(cudaMalloc(&temporary, count * sizeof(B)), "cudaMalloc temporary BF16 matrix");
        cuda_ok(cudaMemcpy(temporary, source.w, count * sizeof(B), cudaMemcpyHostToDevice),
                "upload temporary BF16 matrix");
        float* target = arena.floats(count);
        k_bf16_to_f32<<<(count + THREADS - 1) / THREADS, THREADS>>>(temporary, target, count);
        cuda_ok(cudaGetLastError(), "expand BF16 matrix");
        // `cudaFree` would synchronize this stream too, but spelling it out makes the lifetime
        // rule obvious to a reader: the conversion must finish before its BF16 input is freed.
        cuda_ok(cudaDeviceSynchronize(), "finish BF16 matrix expansion");
        cuda_ok(cudaFree(temporary), "free temporary BF16 matrix");
        return {target, source.rows, source.cols};
    }
};
...
void gemv(CudaWork& work, const DeviceLinear& weight, const float* input, float* output) {
    // 权重实际是 row-major W[rows,cols]。cuBLAS 只看 column-major，故把同一段内存解释为
    // W^T[cols,rows]，再请求 transpose：output[rows,1] = W * input[cols,1]。
    // 第一版刻意不把 input 舍入 BF16：DeviceLinear 是 checkpoint BF16 值在加载时展开的 FP32
    // 矩阵，activation、累加和和 recurrent state 也都保持 FP32。这使 CUDA 的数学语义直接
    // 对齐 Stage 2 CPU oracle。
    // 以后性能课可以把 input 临时降为 BF16 来启用 Tensor Cores，并为那条不同数值路径另建
    // regression；绝不能让“更快”悄悄改变这里的 correctness contract。
    const float one = 1.0f, zero = 0.0f;
    cublas_ok(cublasSgemv(work.blas, CUBLAS_OP_T, weight.cols, weight.rows, &one,
                           weight.w, weight.cols, input, 1, &zero, output, 1),
              "cublasSgemv W*x");
}

void deltanet_cuda(const DeviceDelta& d, float* conv, float* recurrent, const float* input,
...
void attention_cuda(const DeviceAttention& a, float* keys, float* values, int position,
...
void mlp_cuda(const DeviceLayer& layer, const float* input, CudaWork& w, float* out) {
...
void forward_cuda(const DeviceModel& model, CudaState& state, int token, CudaWork& work,
    ...
    for (int index = 0; index < N; ++index) {
        const DeviceLayer& layer = model.layer[index];
    ...
    // lm_head 与 embedding tied，但其 read-only device copy is FP32 for the same GEMV
    // contract as every other linear.  DeviceModel uploads it separately once at construction.
    gemv(work, model.lm_head, work.n, work.logits);
...
void prefill_cuda(const DeviceModel& model, CudaState& state, CudaWork& work,
    ...
    if (prompt.empty()) cuda_die("prefill", "prompt is empty");
    for (int token : prompt) forward_cuda(model, state, token, work);
}

// decode 只处理一个已经采样出的 token；不混入 sampling、stop string 或 server policy。
void decode_cuda(const DeviceModel& model, CudaState& state, CudaWork& work, int token) {
    forward_cuda(model, state, token, work);
}

void download_logits(CudaWork& work) {
    cuda_ok(cudaMemcpy(work.host_logits.data(), work.logits, static_cast<size_t>(V) * sizeof(float),
                       cudaMemcpyDeviceToHost), "download logits");
}

void trace_logits_cuda(const DeviceModel& model, CudaState& state, CudaWork& work,
                       const std::vector<int>& tokens, const char* path) {
    FILE* output = std::fopen(path, "wb");
    if (!output) cuda_die("trace", "cannot open trace output");
    for (int token : tokens) {
        forward_cuda(model, state, token, work, true);
        if (std::fwrite(work.host_logits.data(), sizeof(float), V, output) != static_cast<size_t>(V)) {
            std::fclose(output);
            cuda_die("trace", "cannot write trace output");
        }
    }
    if (std::fclose(output) != 0) cuda_die("trace", "cannot close trace output");
}

bool same_device_floats(const float* left, const float* right, size_t count) {
    std::vector<float> host_left(count), host_right(count);
    cuda_ok(cudaMemcpy(host_left.data(), left, count * sizeof(float), cudaMemcpyDeviceToHost),
            "download left state");
    cuda_ok(cudaMemcpy(host_right.data(), right, count * sizeof(float), cudaMemcpyDeviceToHost),
            "download right state");
    return std::memcmp(host_left.data(), host_right.data(), count * sizeof(float)) == 0;
}

bool same_cuda_state(const CudaState& left, const CudaState& right) {
    if (left.position != right.position) return false;
    const size_t conv_count = static_cast<size_t>(DQKV) * (CK - 1);
    const size_t recurrent_count = static_cast<size_t>(VH) * KD * VD;
    const size_t kv_count = static_cast<size_t>(left.position) * KVS;
    for (int layer = 0; layer < N; ++layer) {
        if (layer % 4 != 3) {
            if (!same_device_floats(left.conv[layer], right.conv[layer], conv_count) ||
                !same_device_floats(left.recurrent[layer], right.recurrent[layer], recurrent_count)) return false;
        } else if (kv_count &&
                   (!same_device_floats(left.keys[layer], right.keys[layer], kv_count) ||
                    !same_device_floats(left.values[layer], right.values[layer], kv_count))) {
            return false;
        }
    }
    return true;
...
void state_check_cuda(const DeviceModel& model, const std::vector<int>& prompt,
                      const std::vector<int>& suffix) {
    CudaState via_api, direct;
    CudaWork api_work, direct_work;
    prefill_cuda(model, via_api, api_work, prompt);
    for (int token : suffix) decode_cuda(model, via_api, api_work, token);
    for (int token : prompt) forward_cuda(model, direct, token, direct_work);
    for (int token : suffix) forward_cuda(model, direct, token, direct_work);
    download_logits(api_work);
    download_logits(direct_work);
    if (!same_cuda_state(via_api, direct) ||
        std::memcmp(api_work.host_logits.data(), direct_work.host_logits.data(), V * sizeof(float)) != 0) {
        cuda_die("state-check", "prefill/decode state differs from direct forward");
    }
}
...
void generate_cuda(const DeviceModel& model, CudaState& state, CudaWork& work,
    ...
    for (int step = 0; step < count; ++step) {
        ...
        if (step + 1 < count) decode_cuda(model, state, work, next.token);
    }
}
...
int main(int argc, char** argv) {
    ...
    if (std::strcmp(argv[1], "--forward") == 0 && argc == 4) {
        ...
        prefill_cuda(model, state, work, parse_ids(argv[3]));
        const DeviceTop next = argmax_cuda(work);
    ...
    if (std::strcmp(argv[1], "--logits") == 0 && argc == 4) {
        ...
        DeviceModel model(argv[2]); CudaState state; CudaWork work;
        prefill_cuda(model, state, work, parse_ids(argv[3]));
        download_logits(work);
        dump_logits(work.host_logits);
        return 0;
    }
    if (std::strcmp(argv[1], "--trace-logits") == 0 && argc == 5) {
        DeviceModel model(argv[2]); CudaState state; CudaWork work;
        trace_logits_cuda(model, state, work, parse_ids(argv[3]), argv[4]);
        return 0;
    }
    if (std::strcmp(argv[1], "--state-check") == 0 && argc == 5) {
        DeviceModel model(argv[2]);
        state_check_cuda(model, parse_ids(argv[3]), parse_ids(argv[4]));
        std::printf("state-check: passed\n");
        return 0;
    }
