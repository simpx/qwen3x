#!/usr/bin/env python3
"""Compare the Stage 3 CUDA + cuBLAS path with Stage 1 official HF vectors."""
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--max-abs-error", type=float,
                        help="override the versioned CUDA tolerance recorded in vectors.json")
...
def trace(engine: Path, weights: Path, input_ids: np.ndarray) -> np.ndarray:
    ...
    with tempfile.TemporaryDirectory(prefix="qwen35-cuda-trace-") as directory:
...
def compare(name: str, official: np.ndarray, actual: np.ndarray, limit: float) -> None:
    ...
    if maximum > limit:
        raise RuntimeError(f"{name}: max_abs_error {maximum:.6g} exceeds CUDA limit {limit:.6g}")
...
def main() -> None:
    ...
    if metadata.get("device") != "cuda:0":
        raise SystemExit("Stage 3 needs the CUDA official vector bundle, not a CPU bundle")
    vector_limit = metadata.get("comparison_tolerances", {}).get("cuda_max_abs_error")
    limit = args.max_abs_error if args.max_abs_error is not None else vector_limit
    if not isinstance(limit, (int, float)) or limit <= 0:
        raise SystemExit("vectors.json has no valid CUDA max_abs_error contract")
    ...
    print("Stage 3 CUDA reference: passed")
