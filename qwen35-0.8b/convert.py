...
这不是通用模型转换器。它只做一件事：以同目录 qwen3x.cpp 前向执行所需的
...
字节布局，以及 qwen3x.cpp 中 Model 构造函数下一次 Reader.take() 应读到的内容。
"""
...
MAGIC = b"Q3XCOUR\0"  # 防止把任意文件误当作本课程 model.bin。
HEADER = struct.Struct("<8sII")  # magic、格式版本、保留字段，共固定 16 字节。

# 以下常数与同目录 qwen3x.cpp 完全重复，是刻意的“显式固定模型”，不是遗漏。
