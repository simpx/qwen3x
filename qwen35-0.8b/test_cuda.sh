...
# greedy decode 都放在一起检查。CUDA 的允许 logit 误差来自并行 reduction 顺序不同。
set -euo pipefail
...
python3 pack_weights.py "$model_dir" "$model_bin" >/dev/null
cpu_forward=$(./qwen35 --forward "$model_bin" 248044,198,198)
...
if abs(float(cpu.group(2)) - float(cuda.group(2))) > 1e-3:
    raise SystemExit(f"logit mismatch: CPU {cpu.group(2)} != CUDA {cuda.group(2)}")
PY

cpu_generate=$(./qwen35 --generate "$model_bin" 248044,198,198 8)
