...
// Embedding is one row of W[V,H], decoded into FP32; it performs no dot product.
void embed(const Linear& table, int token, FP32* out) {
    ...
    if (q3x_cpu::try_embed(table.w, table.type, table.cols, token, out)) return;
    const BF16* row = static_cast<const BF16*>(table.w) + static_cast<size_t>(token) * table.cols;
...
// y[rows] = W[rows,cols] * x[cols]. The BF16 baseline stays visible here.
...
// Experts are contiguous matrices W[experts,rows,cols]; select one, then use mv.
void expert_mv(const ExpertLinear& w, int expert, const FP32* x, FP32* y) {
    Q3X_ASSERT(expert >= 0 && expert < w.experts,
               "expert=%d count=%d", expert, w.experts);
    size_t bytes = static_cast<size_t>(w.rows) * w.cols * sizeof(BF16);
    if (w.type != q3x_model::MATRIX_BF16)
        Q3X_ASSERT(q3x_cpu::quantized_bytes(w.type, w.rows, w.cols, &bytes),
                   "invalid expert matrix layout type=%u", w.type);
    Linear selected {static_cast<const uint8_t*>(w.w) + expert * bytes,
...
void moe(const MoeWeights& weights, const FP32* input, Work& work,
    ...
    for (int slot = 0; slot < c.top_k; ++slot) {
        ...
        expert_mv(weights.gate_up, expert, input, work.ffn_gate_up);
        for (int i = 0; i < c.I; ++i)
            work.ffn_gate[i] = silu(work.ffn_gate[i]) * work.ffn_up[i];
        expert_mv(weights.down, expert, work.ffn_gate, work.delta_output);
...
bool Model::load(const char* path, const char** error) {
    ...
    auto linear = [&](int rows, int cols, q3x_model::MatrixType type) {
        if (type == q3x_model::MATRIX_BF16)
            return Linear {take<BF16>(file, file_size, cursor,
                           ...
                           rows, cols, type};
        size_t bytes = 0;
        if (!*error && !q3x_cpu::quantized_bytes(type, rows, cols, &bytes))
            *error = "invalid quantized matrix layout";
        return Linear {take<uint8_t>(file, file_size, cursor, bytes, error),
                       rows, cols, type};
    };
    ...
    auto expert = [&](int rows, int cols) {
        const Linear all = model_linear(c.experts * rows, cols);
        return ExpertLinear {all.w, c.experts, rows, cols, all.type};
    };
