...
engine.cpp     权重加载顺序、State/Work、模型算子和完整 Qwen forward
arch/cpu.cpp   CPU 量化标量实现、架构选择、线程调度与扩展入口
...
- `engine.cpp` 保留 Linear、BF16 权重读取、embed/mv/dot 的朴素公式和唯一 CPU forward，
  使 BF16 主路径在单文件内自洽。只通过 `q3x_cpu` 接入扩展：try_embed/try_mv 先尝试架构
  加速或量化实现；完成计算返回 true，BF16 回退返回 false 且不修改输出，随后执行 engine
  内的公式。量化格式大小也由 CPU 扩展入口提供，engine 不包含量化格式分派。
- 平台条件、SIMD intrinsic 和线程调度放在 `arch/`。量化格式大小与标量 embed/mv
  集中在 `arch/cpu.cpp`，格式标识表示完整编码（例如 Q4_0），共享 block 布局保留在
  q4.h/q8.h。权重所有权、tensor 加载顺序、alignment 和 EOF 检查仍在 engine。
  新增格式先补充明确的标量实现及数值测试。
  `CPU_OPT=0` 使用独立目录构建，只关闭架构加速，量化支持始终保留。
...
- diff 按独立行为和依赖关系拆成小而内聚的单元；同一文件可以分属多个提交，同一行为
  所需的实现、测试和说明应一起提交。
...
本流程适用于 AI、`/goal` 或一次较长开发完成后的改动整理。目标是先处理容易审阅的辅助
改动，再集中精力阅读少数几个完整的核心提交；不按开发时间顺序堆积提交。

1. **备份与拆分**：先检查 tracked/untracked 改动及归属，将待整理的完整版本保存在独立
   备份分支，确认没有遗漏后再整理主线。必要的备份快照提交只用于保存代码，不代表主线
   审阅通过；不纳入无关用户改动、敏感文件或 build 产物。列出拟提交的顺序、每个提交
   解决的问题、涉及范围、依赖、验证方式和未解决问题。
   主线每次只展开当前一个提交单元，作为未暂存的 dirty diff 供用户在 IDE 中阅读；其余
   改动留在备份分支。解释当前 diff 并完成适用验证后，等待用户明确确认这一份改动，
   才将它 stage 并 commit。用户要求“开始整理/提交当前 dirty”只启动此流程，不授权
   连续提交整批改动。当前提交确认并完成后，再展开下一份 dirty diff，重复审阅。
   用户要求修改时继续调整当前 diff；`/goal` 完成不自动触发主线提交，push 仍需单独授权。
2. **辅助改动优先**：能独立使用和验证的开发工具、依赖准备、配置、清理或独立 bug fix
   可以先提交。按行为分别归组，不把所有“非 engine 文件”塞进一个杂项提交。引用尚未
   加入的实现的构建目标、启动配置、文档和测试，随对应功能提交或放在其后。
3. **核心功能保持完整**：模型计算、权重格式、Session/cache 等核心行为保留为少数几个
   可从输入读到输出的提交。必须一起工作的 loader、forward、接口适配、构建接入和验收
   测试属于同一个功能单元，不为了缩小 diff 留下无法运行的中间状态。
4. **CPU 与后端一起审阅、一起提交**：CPU 是模型数学和状态语义的可读基准。新增或修改
   共同的模型行为时，在同一核心提交中包含所需的 CPU 实现及受影响后端适配，说明 shape、
   公式、权重布局和状态变化的对应关系；不先合入后端语义变化，再留待未来补 CPU 对齐。
   新增后端或纯架构优化若复用现有 CPU 语义，无需制造 CPU diff，但提交说明必须指出对应
   CPU 路径和对齐结果。尚不支持的后端应明确拒绝，不能静默沿用不兼容行为。
5. **验证随实现提交**：核心提交包含相关回归测试和可复现的数值验证入口。相同权重和计算
   路径比较 logits、生成及 cache/checkpoint 行为；量化格式或计算精度不同时，说明差异，
   以同权重 reference 验证数值，并单独核对 CPU 数学语义，不要求不同量化逐位相等。
   未运行的检查、容差和已知失败如实记录；测试源码存在不代表验收通过。
6. **性能优化单独审阅**：已有可运行且正确的基线时，将可分离的性能优化放在后续提交，
   携带正确性对照和前后 benchmark。不为拆提交重建虚假的开发历史，也不把尚未验证的
   优化包装成已验收结果。最终使用说明和整体评测结论在相关功能就绪后收尾。
7. **逐提交核验**：每个提交在已有前置提交之上都应能构建并通过适用检查；不能只验证包含
   全部未提交文件的工作区。必要时使用隔离工作树验证中间提交，保持用户改动原样。提交前
   检查 staged diff，提交后报告 hash、用途、验证结果及仍待审阅的核心改动。

提交说明以具体问题和最终行为开头；核心提交补充 CPU 对应路径、数值/性能证据与已知
限制。文档、测试和实现按行为归组，不因文件类型不同而机械拆开。
